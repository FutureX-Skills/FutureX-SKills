...
const RATE_LIMIT_BASE_DELAY_MS = 60_000; // 1 分钟起步，指数退避
...
async function fetchYouTubeContent(podcasts, apiKey, state, errors) {
  ...
  // Fetch transcript — 失败时优雅降级，保留视频信息但 transcript 为空
  let transcript = '';
  try {
    ...
    if (!transcriptRes.ok) {
      errors.push(`YouTube: Failed to get transcript for ${selected.videoId}: HTTP ${transcriptRes.status} (video info preserved)`);
    } else {
      const transcriptData = await transcriptRes.json();
      transcript = transcriptData.content || '';
    }
  } catch (err) {
    errors.push(`YouTube: Error fetching transcript for ${selected.videoId}: ${err.message} (video info preserved)`);
  }

  // Mark as seen — 即使 transcript 失败也标记，避免重复尝试
...
async function fetchXContent(xAccounts, bearerToken, state, errors) {
  ...
  for (let ai = 0; ai < xAccounts.length; ai++) {
    const account = xAccounts[ai];
      ...
      if (!res.ok) {
        if (res.status === 429) {
          if (rateLimitRetries >= RATE_LIMIT_MAX_RETRIES) {
            errors.push(`X API: Rate limited ${RATE_LIMIT_MAX_RETRIES + 1} times, skipping remaining ${xAccounts.length - ai} accounts`);
            break;
          }
          // 指数退避：1分钟 → 2分钟
          const delay = RATE_LIMIT_BASE_DELAY_MS * Math.pow(2, rateLimitRetries);
          rateLimitRetries++;
          errors.push(`X API: Rate limited, waiting ${delay / 1000}s before retry (attempt ${rateLimitRetries}/${RATE_LIMIT_MAX_RETRIES})`);
...
async function main() {
  ...
  if (!tweetsOnly) {
    ...
    const ytErrors = errors.filter(e => e.startsWith('YouTube'));
    const podcastFeed = {
      generatedAt: new Date().toISOString(),
      lookbackHours: PODCAST_LOOKBACK_HOURS,
      podcasts,
      stats: { podcastEpisodes: podcasts.length },
      errors: ytErrors.length > 0 ? ytErrors : undefined
    };
