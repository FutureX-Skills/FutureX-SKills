...
async function sendTelegram(text, botToken, chatId) {
  ...
  for (const chunk of chunks) {
    ...
    if (!res.ok) {
      const err = await res.json().catch(() => ({ description: `HTTP ${res.status}` }));
      // Markdown 解析失败时，去掉 parse_mode 重试
      if (err.description && err.description.includes("can't parse")) {
        ...
        if (!retryRes.ok) {
          const retryErr = await retryRes.json().catch(() => ({ description: `HTTP ${retryRes.status}` }));
          throw new Error(`Telegram API error (retry): ${retryErr.description}`);
        }
      } else {
