...
// --- 自动发现 Chrome 调试端口 ---
// 通过 HTTP /json/version 拿 Chrome 最新 WebSocket URL（优先于文件，避免 UUID 过期）
async function queryHttpApi(port) {
...
async function discoverChromePort() {
  // 1. 优先查 HTTP API（总是返回最新 UUID，不会过期）
  const commonPorts = [9222, 9229, 9333];
  for (const port of commonPorts) {
    const wsPath = await queryHttpApi(port);
    if (wsPath) {
      console.log(`[CDP Proxy] HTTP API 发现 Chrome 调试端口: ${port} (wsPath: ${wsPath})`);
      return { port, wsPath };
    }
  }

  // 2. 回退：读 DevToolsActivePort 文件
  const possiblePaths = [];
