...
```bash
# 克隆仓库
git clone https://github.com/FutureX-Skills/FutureX-SKills.git ~/.openclaw/workspace/skills/pdf-ocr-skill

# 安装依赖
pip install pymupdf pillow requests python-dotenv

# 可选：安装 RapidOCR（推荐用于本地识别）
pip install rapidocr_onnxruntime
```

> **前提条件**：Python3 + pip，可选 Silicon Flow API Key（用于云端高精度 OCR）
...
支持双引擎的 PDF OCR 识别工具，可从影印版 PDF 和图片中提取文字。提供本地（RapidOCR）和云端（硅基流动）两种识别引擎。

### 核心能力

- **双引擎支持**：本地 RapidOCR（免费）和硅基流动 API（高精度）
- **PDF 识别**：自动将 PDF 页面转换为图片进行 OCR
- **图片识别**：支持 JPG、PNG、BMP、GIF、TIFF、WEBP 等格式
- **自动切换**：RapidOCR 失败时自动切换到云端引擎
- **中英文支持**：同时支持中文和英文识别
...
# 创建处理器（默认使用 RapidOCR 本地引擎）
processor = PDFOCRProcessor()

# 执行 PDF OCR
result = processor.ocr_pdf('path/to/scanned.pdf')
