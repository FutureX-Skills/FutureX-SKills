...
```bash
# 克隆仓库
git clone https://github.com/FutureX-Skills/FutureX-SKills.git ~/.openclaw/workspace/skills/tencentcloud-lighthouse-skill

# 首次使用：自动设置
# 1. 获取腾讯云 SecretId 和 SecretKey
#    访问 https://console.cloud.tencent.com/cam/capi
# 2. 运行设置脚本
{baseDir}/scripts/setup.sh --secret-id "your-secret-id" --secret-key "your-secret-key"
```

> **前提条件**：Node.js（用于 mcporter），腾讯云 API 密钥
...
腾讯云轻量应用服务器（Lighthouse）管理工具，通过 mcporter + MCP Server 提供实例管理、监控告警、防火墙配置、远程命令执行等功能。

### 核心能力

- **实例管理**：查询、启动、重启实例，查看流量包/套餐
- **监控告警**：获取 CPU、内存、带宽等多指标监控数据，设置告警策略
- **防火墙**：增删改查防火墙规则，管理防火墙模板
- **远程命令**：通过 TAT 在实例上执行命令
- **自检诊断**：自动检测网络、防火墙、存储、状态问题
...
- 除 `describe_regions` 外，所有操作都需要 Region 参数
- 危险操作（防火墙修改、命令执行）前需要先确认
- 命令长度限制 2048 字符
